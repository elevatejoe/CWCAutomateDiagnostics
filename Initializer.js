...
function getInstallerToken (osName) {
  if (osName.match('Windows')) {
    return extensionContext.settingValues.InstallerToken
  } else if (osName.match('Mac OS')) {
    return extensionContext.settingValues.InstallerTokenMac
  } else {
    return extensionContext.settingValues.InstallerTokenLinux
  }
}
...
SC.event.addGlobalHandler(SC.event.RefreshTab, function (eventArgs) {
  if (isMyTab(eventArgs.tabName)) {
    ...
    try {
      SC.ui.findDescendent(eventArgs.container, function (e) {
        return e._commandName == 'RestartAutomate'
      })._commandArgument = {
    ...
    try {
      SC.ui.findDescendent(eventArgs.container, function (e) {
        return e._commandName == 'ReinstallAutomate'
      })._commandArgument = {
...
SC.event.addGlobalHandler(SC.event.ExecuteCommand, function (eventArgs) {
  ...
  switch (eventArgs.commandName) {
    ...
    case 'ReinstallAutomate':
      //sendCommand(eventArgs);
      showReinstallPrompt(eventArgs.commandArgument.operatingSystemName)
      break
    case 'RestartAutomate':
      sendCommand({
        commandArgument: {
          type: 'RestartAutomate',
          operatingSystemName: eventArgs.commandArgument.operatingSystemName
        }
      })
      break
    case 'GetInfoPer':
      ...
      var windowsSessionIDs = Array.prototype.map
        .call(checkedOrSelectedSessions, function (s) {
          if (s.GuestOperatingSystemName.includes('Windows')) return s.SessionID
        })
        .filter(function (s) {
          return s !== undefined
        })
...
function displayAutomateDiagInfo (latestDiagnosticEvent, baseTime) {
  try {
    ...
    $('lastUpdateContainer').innerHTML =
      ...
      new Date(baseTime - latestDiagnosticEvent.Time).toLocaleString()
  } catch (e) {
...
function displayDataJson (json) {
  ...
  if ('locationid' in json) {
    ...
    SC.ui.addElement($('locationid_row'), 'td', {
      id: 'locationid',
      innerHTML: locationid_status + ' ' + json['locationid']
    })
  }
...
function getAutomateCommandText (headers) {
  console.log(
    'OS: ' + headers.OperatingSystem + ' | Command: ' + headers.DiagnosticType
  )
  ...
  switch (
    ...
    case 'ps/Windows/powershell/json/Automate':
      return (
        "$WarningPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Try { (new-object Net.WebClient).DownloadString('" +
    ...
    case 'ps/Windows/powershell/json/RestartAutomate':
      return (
        "$WarningPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (new-object Net.WebClient).DownloadString('" +
        getLTPoSh() +
    ...
    case 'sh/Linux/bash/json/Automate':
      return (
        ...
        '; CURL=$(command -v curl); WGET=$(command -v wget); if [ ! -z $CURL ]; then echo $($CURL -s $url | python3 - -e); else echo $($WGET -q -O - --no-check-certificate $url | python3 - -e); fi'
      )
    ...
    case 'sh/Linux/bash/json/RestartAutomate':
      return 'service ltechagent stop ; service ltechagent start'

    case 'sh/Mac/bash/json/RestartAutomate':
      return 'launchctl stop com.labtechsoftware.LTSvc ; launchctl start com.labtechsoftware.LTSvc'

    case 'sh/Linux/bash/json/ReinstallAutomate':
      var txtlocationid = $('#locationidreinstall').value
      var txtinstallertoken = $('#installertoken').value
      if (isNaN(txtlocationid)) {
        txtlocationid = '1'
      ...
      return (
        //'cd /usr/local/ltechagent; ./uninstaller.sh; cd /tmp; rm -f LTechAgent.zip; rm -r -f LTechAgent; curl -sS -o LTechAgent.zip https://' +
		'cd /tmp; rm -f LTechAgent.zip; rm -r -f LTechAgent; curl -sS -o LTechAgent.zip https://' +
        getLTServer() +
        '/LabTech/Deployment.aspx?InstallerToken=' +
        txtinstallertoken +
        '; unzip LTechAgent.zip; cd LTechAgent; chmod +x ./install.sh; sudo ./install.sh; service ltechagent status; ps -ax | grep ltech'
      )

    case 'sh/Mac/bash/json/ReinstallAutomate':
      var txtlocationid = $('#locationidreinstall').value
      var txtinstallertoken = $('#installertoken').value
      if (isNaN(txtlocationid)) {
        txtlocationid = '1'
      ...
      return (
        //'sudo sh /usr/local/ltechagent/uninstaller.sh; mkdir /tmp/ltagent; cd /tmp/ltagent; curl -sS -O ltagent.zip https://' +
		'mkdir /tmp/ltagent; cd /tmp/ltagent; curl -sS -O ltagent.zip https://' +
        getLTServer() +
        '/LabTech/Deployment.aspx?InstallerToken=' +
        txtinstallertoken +
        '; unzip ltagent.zip; sudo installer -pkg ltsvc.mpkg -target /; rm *; cd .; rmdir /tmp/ltagent; sudo launchctl list | grep com.labtechsoftware'
      )
